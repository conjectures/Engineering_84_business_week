...
SELECT pr.SupplierID, sp.CompanyName, sp.Country, pr.ProductID, pr.ProductName, pr.QuantityPerUnit FROM Products as pr
    INNER JOIN Suppliers as sp ON pr.SupplierID = sp.SupplierID
    WHERE pr.QuantityPerUnit LIKE '%bottle%';


--1.4 - 
SELECT prod.CategoryID, cat.CategoryName, COUNT(*) AS "Item Number" FROM Products as prod 
...
SELECT rg.RegionID, rg.RegionDescription, FORMAT(ROUND(SUM(odt.Quantity * odt.UnitPrice),-5), '###,###,###') as "Total Sales (Approx.)" FROM Region as rg
...
SELECT COUNT(*) AS "Orders with Freight above 100 and destination US or UK" FROM Orders
